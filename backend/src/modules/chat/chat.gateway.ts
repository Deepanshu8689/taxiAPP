...
export class ChatGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  handleJoinChat(
    @MessageBody() initialChat: any,
    @ConnectedSocket() socket: Socket
  ) {
    console.log("initialChat: ", initialChat)
    if (initialChat) {
      const roomId = `${initialChat.ticketId}_supportChat`
      console.log("roomId: ", roomId)
      socket.join(roomId)
    }
  }

  @SubscribeMessage('BE-send-message-to-support')
  handleSendMessageUser(
    @MessageBody() data: any,
    @ConnectedSocket() socket: Socket
  ) {
    const { chatId, sentMessage, ticketId } = data
    ...
    const roomId = `${ticketId}_supportChat`

    this.io.to(roomId).emit('FE-receive-message-from-support', sentMessage)

  }


  @SubscribeMessage('BE-reply-to-user')
  handleSendMessageSupport(
    @MessageBody() data: any,
    @ConnectedSocket() socket: Socket
  ) {
    const { chatId, sentMessage, ticketId } = data
  ...
  @SubscribeMessage('BE-generate-ticket')
  handleGenerateTicket(
    @MessageBody() data: any,
    @ConnectedSocket() socket: Socket
  ) {
    console.log("data: ", data)
    this.io.emit('FE-generate-ticket', data)
  }
}